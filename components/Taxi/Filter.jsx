...
const Filter = ({vehicles,onFilterChange}) => {
  ...
  const availableCarTypes = [...new Set(vehicles.map(v => v.carType).filter(Boolean))];
      ...
      <div className="relative w-full">
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 pointer-events-none">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            <path fillRule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1-.708.708L8 5.707l-2.646 2.647a.5.5 0 0 1-.708-.708l3-3z"/>
          </svg>
        </div>
      </div>
      <div className="relative w-full">
      ...
      <div className="space-y-3">
        {availableModels.map(model => (
          <div key={model} className="flex items-center">
            ...
            />
            <label htmlFor={`model-${model}`} className="text-base font-semibold cursor-pointer">{model}</label>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="space-y-3">
        {availableFuelTypes.map(type => (
          <div key={type} className="flex items-center">
            ...
            />
            <label htmlFor={`fuel-${type}`} className="text-base font-semibold cursor-pointer">{type}</label>
          </div>
        ))}
      </div>
    </div>
        ...
        {availableCarTypes.map(type => (
          <div key={type} className="flex items-center">
            <input
              ...
              className="w-5 h-5 border-2 border-gray-300 rounded mr-3 cursor-pointer"
            />
            <label htmlFor={`type-${type}`} className="text-base font-semibold cursor-pointer">{type}</label>
          </div>
        ))}
      </div>
    </div>
